    ...
    callbacks: {
        async jwt({ token, user, trigger, session }: { token: JWT; user?: User; trigger?: "signIn" | "signUp" | "update"; session?: any }) {
            ...
            if (user) {
                ...
                token.org_node_id = u.org_node_id ?? u.user_info?.org_node_id;
                token.wilayah_name = u.wilayah_name ?? u.user_info?.wilayah_name;
                token.wilayah_code = u.wilayah_code ?? u.user_info?.wilayah_code;
        ...
        async session({ session, token }: { session: Session; token: JWT }) {
            ...
            session.user.org_node_id = token.org_node_id;
            session.user.wilayah_name = token.wilayah_name;
            session.user.wilayah_code = token.wilayah_code;
