...
const Page = () => {
    ...
    const [dataRekap, setDataRekap] = useState<DataRekap>({
        ...
        show: false,
        adjust: false,
        fileName: `laporan-master-pajak-${new Date().toISOString().slice(0, 10)}`,
        judul1: 'Laporan Master Pajak',
        judul2: ''
    });
    ...
    const formik = useFormik<initValue>({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.kode_cabang) errors.kode_cabang = 'Kode Cabang wajib diisi.';
            if (!data.name || !data.name.trim()) errors.name = 'Nama Pajak wajib diisi.';
            if (!data.tax_type) errors.tax_type = 'Tipe Pajak wajib diisi.';
            if (data.percentage === null || data.percentage === undefined) errors.percentage = 'Persentase wajib diisi.';
            return errors;
        },
        onSubmit: (data) => {
