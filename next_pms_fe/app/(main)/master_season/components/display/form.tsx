...
const Form = ({ state, setState, formik, toast, getData }: FormProps) => {
    ...
    const handleSave = async (input: initValue) => {
        ...
        try {
            ...
            const oBody: Record<string, any> = {
                nama_musim: input.nama_musim,
                kode_cabang: input.kode_cabang,
                tanggal_mulai: input.tanggal_mulai,
                tanggal_selesai: input.tanggal_selesai,
                hari_berlaku: input.hari_berlaku,
                is_active: input.is_active,
                tz: getTzUser()
            };

            if (isEdit) {
                oBody['kode_musim'] = input.kode_musim;
            }

            const vaData = await postData(cEndPoint, oBody, oHeaders);
            const res = vaData.data;

            showSuccess(toast, res.message || 'Berhasil Menyimpan Data Season');
            formik.resetForm();
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const vaCode = state.selectedDatas.map((v) => v.kode_musim);

            const vaData = await postData(apiEndpointDelete, { kode_musim: vaCode, tz: getTzUser() });
            const res = vaData.data;

            showSuccess(toast, res.message || 'Berhasil menghapus data Season.');
                ...
                <form onSubmit={formik?.handleSubmit} className="flex gap-3 flex-column pt-2">
                    <div className="flex gap-2 flex-column w-full">
                        ...
                        <div className="flex flex-column md:flex-row gap-3 w-full mt-2">
                            ...
                            <div className="flex flex-column gap-1 w-full">
                                <label htmlFor="nama_musim" className="font-semibold text-sm">
                                    Nama Season <span className="text-red-500">*</span>
                                </label>
                                <InputText
                                    id="nama_musim"
                                    name="nama_musim"
                                    value={formik?.values.nama_musim || ''}
                                    placeholder="Contoh: High Season 2026, Peak Season"
                                    onChange={(e) => formik?.setFieldValue('nama_musim', e.target.value)}
                                    className={isFormFieldInvalid('nama_musim') ? 'p-invalid w-full' : 'w-full'}
                                />
                                {getFormErrorMessage('nama_musim')}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-column md:flex-row gap-3 w-full mt-2">
                            <div className="flex flex-column gap-1 w-full">
                                <label htmlFor="tanggal_mulai" className="font-semibold text-sm">
                                    Tanggal Mulai
                                </label>
                                <InputText
                                    id="tanggal_mulai"
                                    ...
                                    onChange={(e) => formik?.setFieldValue('tanggal_mulai', e.target.value)}
                                    className="w-full"
                                />
                            </div>
                            <div className="flex flex-column gap-1 w-full">
                                <label htmlFor="tanggal_selesai" className="font-semibold text-sm">
                                    Tanggal Selesai
                                </label>
                                <InputText
                                    id="tanggal_selesai"
                                    ...
                                    onChange={(e) => formik?.setFieldValue('tanggal_selesai', e.target.value)}
                                    className="w-full"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-column gap-1 w-full mt-2">
                            <label htmlFor="hari_berlaku" className="font-semibold text-sm">
                                Hari Berlaku
                            </label>
                            <InputText
                                id="hari_berlaku"
                                name="hari_berlaku"
                                value={formik?.values.hari_berlaku || ''}
                                placeholder="Contoh: Monday,Tuesday atau Senin,Selasa"
                                onChange={(e) => formik?.setFieldValue('hari_berlaku', e.target.value)}
                                className="w-full"
                            />
                        </div>
                ...
                <div className="flex flex-column align-items-center text-center gap-4 py-4">
                    ...
                    <div>
                        <h3 className="font-bold mb-2">
                            {state.selectedDatas.length > 1 ? `Hapus ${state.selectedDatas.length} data Season?` : 'Hapus data Season ini?'}
                        </h3>
                        <p className="text-color-secondary">
                            {state.selectedDatas.length > 1 ? (
                                `Anda akan menghapus ${state.selectedDatas.length} item season.`
                            ) : (
                                <>
                                    Anda akan menghapus season: <br />
                                    <strong>
                                        {state.selectedDatas[0]?.kode_musim || ''} - {state.selectedDatas[0]?.nama_musim || ''}
                                    </strong>
                                </>
                            )}
                            <br />
