...
const Page = () => {
    ...
    const [dataRekap, setDataRekap] = useState<DataRekap>({
        ...
        show: false,
        adjust: false,
        fileName: `laporan-master-cabang-${new Date().toISOString().slice(0, 10)}`,
        judul1: 'Laporan Master Cabang',
        judul2: ''
    });
    ...
    const formik = useFormik<initValue>({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.name || !data.name.trim()) {
                ...
                errors.name = 'Nama Cabang minimal 3 karakter.';
            }

            if (!data.check_in_time) {
                errors.check_in_time = 'Waktu Check-In wajib diisi.';
            }

            if (!data.check_out_time) {
                errors.check_out_time = 'Waktu Check-Out wajib diisi.';
            }
    ...
    const getPrintData = async (apiEndpoint: string) => {
        ...
        try {
            ...
            const formattedData = transformTableData(res.data.data, {
                ...
                excludeKeys: ['id', 'cabang_id', 'logo_url']
            });
