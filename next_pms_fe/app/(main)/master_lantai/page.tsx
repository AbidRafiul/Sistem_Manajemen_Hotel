...
const Page = () => {
    ...
    const [dataRekap, setDataRekap] = useState<DataRekap>({
        ...
        show: false,
        adjust: false,
        fileName: `laporan-master-lantai-${new Date().toISOString().slice(0, 10)}`,
        judul1: 'Laporan Master Lantai',
        judul2: ''
    });
    ...
    const formik = useFormik<initValue>({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.kode_cabang) errors.kode_cabang = 'Cabang wajib diisi.';
            if (!data.kode_gedung) errors.kode_gedung = 'Gedung wajib diisi.';
            if (!data.name || !data.name.trim()) errors.name = 'Nama Lantai wajib diisi.';
            return errors;
        },
        onSubmit: (data) => {
    ...
    const getPrintData = async (apiEndpoint: string) => {
        ...
        try {
            ...
            const formattedData = transformTableData(res.data.data, {
                ...
                excludeKeys: ['id', 'kode_gedung', 'created_at', 'updated_at']
            });
