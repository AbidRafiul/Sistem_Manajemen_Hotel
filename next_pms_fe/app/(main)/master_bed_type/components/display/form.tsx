...
const Form = ({ state, setState, formik, toast, getData }: FormProps) => {
    const handleSave = async (input: initValue) => {
        ...
        try {
            ...
            if (isEdit) {
                oBody['kode_bed_type'] = input.kode_bed_type;
            }

            const vaData = await postData(cEndPoint, oBody, oHeaders);
            const res = vaData.data;

            showSuccess(toast, res.message || 'Berhasil Menyimpan Data Bed Type');
            formik.resetForm();
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const vaCode = state.selectedDatas.map((v) => v.kode_bed_type);

            const vaData = await postData(apiEndpointDelete, { kode_bed_type: vaCode, tz: getTzUser() });
            const res = vaData.data;

            showSuccess(toast, res.message || 'Berhasil menghapus data Bed Type.');
    ...
    return (
        <>
            <Dialog
                ...
                header={state.edit ? 'Edit Data Bed Type' : 'Tambah Bed Type Baru'}
                modal
                    ...
                    <div className="flex gap-2 flex-column w-full">
                        ...
                        <div className="flex flex-column gap-1 w-full">
                            <label htmlFor="name" className="font-semibold text-sm">
                                Nama Bed Type <span className="text-red-500">*</span>
                            </label>
                            <InputText
                                id="name"
                                name="name"
                                value={formik?.values.name || ''}
                                placeholder="Contoh: Single, Double, Twin"
                ...
                <div className="flex flex-column align-items-center text-center gap-4 py-4">
                    ...
                    <div>
                        <h3 className="font-bold mb-2">
                            {state.selectedDatas.length > 1 ? `Hapus ${state.selectedDatas.length} data Bed Type?` : 'Hapus data Bed Type ini?'}
                        </h3>
                        <p className="text-color-secondary">
                            {state.selectedDatas.length > 1 ? (
                                `Anda akan menghapus ${state.selectedDatas.length} item bed type.`
                            ) : (
                                <>
                                    Anda akan menghapus bed type: <br />
                                    <strong>
                                        {state.selectedDatas[0]?.kode_bed_type || ''} - {state.selectedDatas[0]?.name || ''}
                                    </strong>
                                </>
                            )}
                            <br />
