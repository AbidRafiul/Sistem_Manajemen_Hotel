...
const Page = () => {
    ...
    const [dataRekap, setDataRekap] = useState<DataRekap>({
        ...
        show: false,
        adjust: false,
        fileName: `laporan-master-rate-plan-${new Date().toISOString().slice(0, 10)}`,
        judul1: 'Laporan Master Rate Plan',
        judul2: ''
    });
    ...
    const formik = useFormik<initValue>({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.name || !data.name.trim()) errors.name = 'Nama Rate Plan wajib diisi.';
            if (!data.tipe_paket) errors.tipe_paket = 'Tipe Paket wajib dipilih.';
            if (data.minimal_malam < 1) errors.minimal_malam = 'Minimal malam harus lebih besar dari 0.';
