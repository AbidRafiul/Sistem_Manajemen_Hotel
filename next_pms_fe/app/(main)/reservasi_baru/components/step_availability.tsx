    ...
    const searchPackages = async () => {
        ...
        try {
            ...
            if (res.data.data) {
                ...
                // Inisialisasi default rate plan per tipe
    ...
    const toggleRoomSelection = (tk: any, rm: any) => {
        if (rm.status !== 'available') {
            if (rm.status === 'occupied') {
                ...
                showError(toast, `Kamar ${rm.nomor_kamar} berstatus Kotor / Belum Lolos Inspeksi. Perlu dibersihkan sebelum check-in.`);
            } else if (rm.status === 'maintenance') {
                showError(toast, `Kamar ${rm.nomor_kamar} sedang dalam perbaikan/pemeliharaan (maintenance).`);
            }
            return;
        }
        ...
        // Kompatibilitas single-room backward
        if (updated.length > 0) {
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {roomsList.length > 0 ? (
                                                        roomsList.map((rm: any, idx: number) => {
                                                            ...
                                                            if (isSelected) {
                                                                // Terpilih (Hijau solid dengan checkmark)
                                                                ...
                                                                // Perlu Dibersihkan (Kuning/Oranye muda)
