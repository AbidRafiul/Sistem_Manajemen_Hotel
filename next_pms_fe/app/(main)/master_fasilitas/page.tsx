...
const Page = () => {
    ...
    const [dataRekap, setDataRekap] = useState<DataRekap>({
        ...
        show: false,
        adjust: false,
        fileName: `laporan-master-fasilitas-${new Date().toISOString().slice(0, 10)}`,
        judul1: 'Laporan Master Fasilitas',
        judul2: ''
    });
    ...
    const formik = useFormik<initValue>({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.name || !data.name.trim()) errors.name = 'Nama Fasilitas wajib diisi.';
            return errors;
        },
        onSubmit: (data) => {
    ...
    const getPrintData = async (apiEndpoint: string) => {
        ...
        try {
            ...
            const formattedData = transformTableData(res.data.data, {
                ...
                excludeKeys: ['id', 'kode_cabang', 'created_at', 'updated_at']
            });
