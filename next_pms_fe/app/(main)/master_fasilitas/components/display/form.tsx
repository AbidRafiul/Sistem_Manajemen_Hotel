...
const Form = ({ state, setState, formik, toast, getData }: FormProps) => {
    ...
    const handleSave = async (input: initValue) => {
        ...
        try {
            ...
            if (isEdit) {
                oBody['kode_fasilitas'] = input.kode_fasilitas;
            }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const vaCode = state.selectedDatas.map((v) => v.kode_fasilitas);
    ...
    return (
        <>
            <Dialog
                ...
                header={state.edit ? 'Edit Data Fasilitas' : 'Tambah Fasilitas Baru'}
                modal
                    ...
                    <div className="flex gap-2 flex-column w-full">
                        ...
                        <div className="flex flex-column gap-1 w-full">
                            ...
                            <InputText
                                ...
                                placeholder="Contoh: Kolam Renang, WiFi, Parkir"
                ...
                <div className="flex flex-column align-items-center text-center gap-4 py-4">
                    ...
                    <div>
                        <h3 className="font-bold mb-2">
                            {state.selectedDatas.length > 1 ? `Hapus ${state.selectedDatas.length} data Fasilitas?` : 'Hapus data Fasilitas ini?'}
                        </h3>
                        <p className="text-color-secondary">
                            {state.selectedDatas.length > 1 ? (
                                `Anda akan menghapus ${state.selectedDatas.length} item fasilitas.`
                            ) : (
                                <>
                                    Anda akan menghapus fasilitas: <br />
                                    <strong>
                                        {state.selectedDatas[0]?.kode_fasilitas || ''} - {state.selectedDatas[0]?.name || ''}
                                    </strong>
                                </>
                            )}
                            <br />
