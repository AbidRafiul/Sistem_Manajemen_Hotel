 ...
 * @project Standard
 * @file task_complete.js
 * @description Endpoint Complete Housekeeping Task
  ...
  try {
    ...
    if (existing.status !== 'in_progress') {
        return res.status(400).json({
            ...
            message: `Hanya task dengan status in_progress yang dapat diselesaikan. Status saat ini: ${existing.status}`,
            datetime: formatDateSystem(),
        });
    }
    ...
    await DB.transaction(async (trx) => {
      ...
      // 2. Update status kamar menjadi inspection jika kamar ditemukan
      if (kamar) {
          await trx("mst_kamar")
          ...
          await ChangesLog(
            {
              ...
              description: "Update status kamar menjadi inspection",
              dataBefore: kamar,
              dataAfter: { ...kamar, housekeeping_status: 'inspection' },
              user: user_id ? String(user_id) : "system",
            },
            trx
          );
...
router.patch("/:id/complete", handleComplete);
