 ...
 * @project Standard
 * @file mst_navigation_update.js
 * @description Endpoint untuk memperbarui data master navigasi sidebar berdasarkan id
 *
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        await DB("mst_navigation")
            ...
            .update({
                menu: oPayload.menu,
                tz: oPayload.tz || existing.tz || "UTC",
                updated_at: formatDateSystem(),
            });

        return res.status(200).json({
            status: status.SUKSES,
            message: "Data navigasi berhasil diperbarui",
            datetime: formatDateSystem(),
        });
    } catch (error) {
        const oResult = {
