...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery.clone().select(selectFields).orderBy(`hre.${sortField}`, sortOrder).limit(perPage).offset((page - 1) * perPage);
    } else {
    ...
    Logging(error, { file: "master/harga_ruang_event/harga_ruang_event_data.js", func: "get", request: oPayload, response: oResult, user: username });
