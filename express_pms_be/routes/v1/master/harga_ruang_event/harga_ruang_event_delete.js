...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_harga_ruang_event").whereIn("kode_harga_ruang_event", oPayload.kode_harga_ruang_event).whereNull("deleted_at").forUpdate();
      if (!records || records.length < 1) { const e = new Error("Data tidak ditemukan"); e.statusCode = 404; throw e; }
      await trx("mst_harga_ruang_event").whereIn("kode_harga_ruang_event", oPayload.kode_harga_ruang_event).update({ is_active: 0, deleted_at: formatDateSystem(), deleted_by: req.auth?.user_id || 1, updated_at: formatDateSystem(), updated_by: req.auth?.user_id || 1 });
      for (const r of records) await ChangesLog({ description: "Soft Delete Master Harga Ruang Event", tableName: "mst_harga_ruang_event", referenceCode: r.kode_harga_ruang_event, action: "DELETE", dataBefore: r, dataAfter: { ...r, is_active: 0, deleted_at: formatDateSystem() }, user: username, tz: oPayload.tz || "UTC" }, trx);
    });
    ...
    Logging(error, { file: "master/harga_ruang_event/harga_ruang_event_delete.js", func: "delete", request: oPayload, response: oResult, user: username });
