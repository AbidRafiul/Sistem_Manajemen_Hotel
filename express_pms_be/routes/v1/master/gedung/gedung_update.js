...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      {
        kode_gedung: Joi.string().required().label("Kode Gedung"),
    ...
    const existing = await DB("mst_gedung")
      .where("kode_gedung", oPayload.kode_gedung)
    ...
    await DB.transaction(async (trx) => {
      ...
      await ChangesLog(
        {
          description: "Update Master Gedung",
          tableName: "mst_gedung",
          referenceCode: oPayload.kode_gedung,
          action: "UPDATE",
          dataBefore: existing,
    ...
    return res
      ...
      .json({
        ...
        message: "Data Master Gedung berhasil diperbarui",
        datetime: formatDateSystem(),
      });
  } catch (error) {
    const oResult = {
    ...
    Logging(error, {
      file: "master/gedung/gedung_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
