...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const baseQuery = DB("mst_gedung as b")
      ...
      .modify((qb) => {
        ...
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(b.nama_gedung) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(b.kode_gedung) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    ...
    Logging(error, {
      file: "master/gedung/gedung_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
