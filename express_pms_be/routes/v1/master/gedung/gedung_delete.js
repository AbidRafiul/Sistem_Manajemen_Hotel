...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_gedung")
        .whereIn("kode_gedung", oPayload.kode_gedung)
      ...
      for (const r of records)
        await ChangesLog(
          {
            description: "Soft Delete Master Gedung",
            tableName: "mst_gedung",
            referenceCode: r.kode_gedung,
            action: "DELETE",
            dataBefore: r,
    ...
    Logging(error, {
      file: "master/gedung/gedung_delete.js",
      func: "delete",
      request: oPayload,
      response: oResult,
      user: username,
