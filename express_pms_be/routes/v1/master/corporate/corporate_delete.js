...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_corporate_account")
        .whereIn("kode_corporate", oPayload.kode_corporate)
      ...
      for (const r of records)
        await ChangesLog(
          {
            description: "Soft Delete Master Corporate",
            tableName: "mst_corporate_account",
            referenceCode: r.kode_corporate,
            action: "DELETE",
            dataBefore: r,
    ...
    Logging(error, {
      file: "master/corporate/corporate_delete.js",
      func: "delete",
      request: oPayload,
      response: oResult,
      user: username,
