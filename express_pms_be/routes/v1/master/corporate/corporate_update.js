...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      {
        ...
        kode_corporate: Joi.string().required().label("Kode"),
        ...
        payment_term_days: Joi.number().min(0).optional().label("Term Pembayaran"),
    ...
    const existing = await DB("mst_corporate_account")
      .where("kode_corporate", oPayload.kode_corporate)
    ...
    const oData = {
      ...
      npwp: oPayload.npwp ?? existing.npwp,
      billing_address: oPayload.billing_address ?? existing.billing_address,
      payment_term_days: oPayload.payment_term_days ?? existing.payment_term_days,
      commission_pct: oPayload.commission_pct ?? existing.commission_pct,
      contact_person: oPayload.contact_person ?? existing.contact_person,
      contact_phone: oPayload.contact_phone ?? existing.contact_phone,
      contact_email: oPayload.contact_email ?? existing.contact_email,
      is_active: oPayload.is_active !== undefined ? oPayload.is_active : existing.is_active,
      updated_by: req.auth?.user_id || 1,
      updated_at: formatDateSystem(),
    };
    await DB.transaction(async (trx) => {
      await trx("mst_corporate_account")
        .where("kode_corporate", oPayload.kode_corporate)
        .update(oData);
      await ChangesLog(
        {
          description: "Update Master Corporate",
          tableName: "mst_corporate_account",
          referenceCode: oPayload.kode_corporate,
          action: "UPDATE",
          dataBefore: existing,
    ...
    Logging(error, {
      file: "master/corporate/corporate_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
