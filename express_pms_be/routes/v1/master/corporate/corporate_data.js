...
router.post("/", async (req, res) => {
  ...
  try {
        ...
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(c.name) LIKE ?", [`%${keyword.toLowerCase()}%`])
              .orWhereRaw("LOWER(c.kode_corporate) LIKE ?", [`%${keyword.toLowerCase()}%`])
              .orWhereRaw("LOWER(c.contact_person) LIKE ?", [`%${keyword.toLowerCase()}%`]);
          });
      });
    const selectFields = [
      "c.id",
    ...
    Logging(error, {
      file: "master/corporate/corporate_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
