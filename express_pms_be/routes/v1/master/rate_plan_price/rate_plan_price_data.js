...
router.post("/", async (req, res) => {
  ...
  const sortField = ["kode_harga_price", "tipe_kamar_name", "rate_plan_name", "season_name", "price", "extra_bed_price", "valid_from", "valid_to", "is_active", "created_at", "updated_at"].includes(oPayload.sortField) ? oPayload.sortField : "updated_at";
  ...
  try {
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery.clone().select(selectFields).orderBy(actualSortField, sortOrder).limit(perPage).offset((page - 1) * perPage);
    } else {
      let actualSortField = `rpp.${sortField}`;
      if (sortField === 'tipe_kamar_name') actualSortField = 'tk.nama_tipe';
      if (sortField === 'rate_plan_name') actualSortField = 'ph.nama_paket';
    ...
    Logging(error, { file: "master/rate_plan_price/rate_plan_price_data.js", func: "get", request: oPayload, response: oResult, user: username });
