...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_lantai")
        .whereIn("kode_lantai", oPayload.kode_lantai)
      ...
      for (const r of records)
        await ChangesLog(
          {
            description: "Soft Delete Master Lantai",
            tableName: "mst_lantai",
            referenceCode: r.kode_lantai,
            action: "DELETE",
            dataBefore: r,
    ...
    Logging(error, {
      file: "master/lantai/lantai_delete.js",
      func: "delete",
      request: oPayload,
      response: oResult,
      user: username,
