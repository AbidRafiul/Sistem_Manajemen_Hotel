...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const baseQuery = DB("mst_lantai as f")
      .join("mst_gedung as b", "f.kode_gedung", "b.kode_gedung")
      .join("mst_cabang as h", "b.kode_cabang", "h.kode_cabang")
      .whereNull("f.deleted_at")
      .modify((qb) => {
        if (targetKodeCabang) {
          qb.where("b.kode_cabang", targetKodeCabang);
        } else {
          qb.whereRaw("1 = 0");
        }
        ...
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(f.nama_lantai) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(f.kode_lantai) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    const selectFields = [
      ...
      "f.kode_gedung",
      "f.kode_lantai",
      "f.nama_lantai as name",
      "f.nomor_lantai",
      "f.is_active",
      "f.created_at",
      "f.updated_at",
      "b.nama_gedung as building_name",
      "h.nama_hotel as hotel_name",
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery
        .clone()
        .select(selectFields)
        .orderBy(sortField === "name" ? "f.nama_lantai" : `f.${sortField}`, sortOrder)
        .limit(perPage)
    ...
    Logging(error, {
      file: "master/lantai/lantai_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
