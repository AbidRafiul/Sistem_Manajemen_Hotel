...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_bed_type").whereIn("kode_bed_type", oPayload.kode_bed_type).whereNull("deleted_at").forUpdate();
      if (!records || records.length < 1) { const e = new Error("Data tidak ditemukan"); e.statusCode = 404; throw e; }
      await trx("mst_bed_type").whereIn("kode_bed_type", oPayload.kode_bed_type).update({ is_active: 0, deleted_at: formatDateSystem(), deleted_by: req.auth?.user_id || 1, updated_at: formatDateSystem(), updated_by: req.auth?.user_id || 1 });
      for (const r of records) await ChangesLog({ description: "Soft Delete Master Bed Type", tableName: "mst_bed_type", referenceCode: r.kode_bed_type, action: "DELETE", dataBefore: r, dataAfter: { ...r, is_active: 0, deleted_at: formatDateSystem() }, user: username, tz: oPayload.tz || "UTC" }, trx);
    });
    return res.status(200).json({ status: status.SUKSES, message: "Data Master Bed Type berhasil dihapus", datetime: formatDateSystem() });
  } catch (error) {
    ...
    Logging(error, { file: "master/bed_type/bed_type_delete.js", func: "delete", request: oPayload, response: oResult, user: username });
