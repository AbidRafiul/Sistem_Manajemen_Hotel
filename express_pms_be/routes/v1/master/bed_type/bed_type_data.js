...
router.post("/", async (req, res) => {
  ...
  const sortField = ["kode_bed_type", "name", "is_active", "created_at", "updated_at"].includes(
    oPayload.sortField
  )
    ? oPayload.sortField
    : "updated_at";
  ...
  try {
    const baseQuery = DB("mst_bed_type as bt")
      .whereNull("bt.deleted_at")
      .modify((qb) => {
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(bt.name) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(bt.kode_bed_type) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery
        .clone()
        .select(selectFields)
        .orderBy(`bt.${sortField}`, sortOrder)
        .limit(perPage)
    ...
    Logging(error, {
      file: "master/bed_type/bed_type_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
