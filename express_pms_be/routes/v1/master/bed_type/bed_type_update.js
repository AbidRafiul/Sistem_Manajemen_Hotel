...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const cValidation = await validatePayload(
      {
        kode_bed_type: Joi.string().required().label("Kode Bed Type"),
    ...
    const existing = await DB("mst_bed_type")
      .where("kode_bed_type", oPayload.kode_bed_type)
    ...
    if (!existing)
      return res
        ...
        .json({
          ...
          message: "Data Bed Type tidak ditemukan",
          datetime: formatDateSystem(),
        });
    const oData = {
      name: oPayload.name,
    ...
    await DB.transaction(async (trx) => {
      ...
      await ChangesLog(
        {
          description: "Update Master Bed Type",
          tableName: "mst_bed_type",
          referenceCode: oPayload.kode_bed_type,
          action: "UPDATE",
          dataBefore: existing,
    ...
    return res
      ...
      .json({
        ...
        message: "Data Master Bed Type berhasil diperbarui",
        datetime: formatDateSystem(),
      });
  } catch (error) {
    const oResult = {
    ...
    Logging(error, {
      file: "master/bed_type/bed_type_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
