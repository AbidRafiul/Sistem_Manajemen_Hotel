...
router.post("/", async (req, res) => {
  ...
  try {
    const baseQuery = DB("mst_musim as s")
      .join("mst_cabang as h", "s.kode_cabang", "h.kode_cabang")
      .whereNull("s.deleted_at")
      .modify((qb) => {
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(s.nama_musim) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(s.kode_musim) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    ...
    Logging(error, {
      file: "master/season/season_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
