...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      const records = await trx("mst_musim")
        .whereIn("kode_musim", oPayload.kode_musim)
      ...
      for (const r of records)
        await ChangesLog(
          {
            description: "Soft Delete Master Season",
            tableName: "mst_musim",
            referenceCode: r.kode_musim,
            action: "DELETE",
            dataBefore: r,
    ...
    Logging(error, {
      file: "master/season/season_delete.js",
      func: "delete",
      request: oPayload,
      response: oResult,
      user: username,
