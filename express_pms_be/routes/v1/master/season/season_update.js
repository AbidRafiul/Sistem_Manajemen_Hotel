...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      {
        ...
        kode_musim: Joi.string().required().label("Kode"),
    ...
    const existing = await DB("mst_musim")
      .where("kode_musim", oPayload.kode_musim)
    ...
    const oData = {
      ...
      tanggal_mulai: oPayload.tanggal_mulai ?? existing.tanggal_mulai,
      tanggal_selesai: oPayload.tanggal_selesai ?? existing.tanggal_selesai,
      hari_berlaku: oPayload.hari_berlaku ?? existing.hari_berlaku,
    ...
    await DB.transaction(async (trx) => {
      ...
      await ChangesLog(
        {
          description: "Update Master Season",
          tableName: "mst_musim",
          referenceCode: oPayload.kode_musim,
          action: "UPDATE",
          dataBefore: existing,
    ...
    Logging(error, {
      file: "master/season/season_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
