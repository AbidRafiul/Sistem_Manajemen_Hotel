...
router.post("/", async (req, res) => {
  ...
  const sortField = ["kode_pajak","kode_cabang","cabang_name","name","tax_type","percentage","is_active","created_at","updated_at"].includes(oPayload.sortField) ? oPayload.sortField : "updated_at";
  const sortOrder = oPayload.sortOrder || "desc";
  try {
    const baseQuery = DB("mst_tax as t").join("mst_cabang as h", "t.kode_cabang", "h.kode_cabang").whereNull("t.deleted_at").modify(qb => {
      if (keyword) qb.where(function() { this.whereRaw("LOWER(t.name) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw("LOWER(t.kode_pajak) LIKE ?", [`%${keyword.toLowerCase()}%`]); });
    });
    const selectFields = ["t.id","t.kode_pajak","t.kode_cabang","h.nama_hotel as cabang_name","t.name","t.tax_type","t.percentage","t.is_compounding","t.is_active","t.created_at","t.updated_at"];
    ...
    Logging(error, { file: "master/pajak/pajak_data.js", func: "get", request: oPayload, response: oResult, user: username });
