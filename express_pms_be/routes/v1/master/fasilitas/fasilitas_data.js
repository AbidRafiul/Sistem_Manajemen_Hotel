...
router.post("/", async (req, res) => {
  ...
  try {
    const baseQuery = DB("mst_fasilitas as f")
      .join("mst_cabang as c", "f.kode_cabang", "c.kode_cabang")
      .whereNull("f.deleted_at")
      .modify((qb) => {
        if (oPayload.kode_cabang) qb.where("f.kode_cabang", oPayload.kode_cabang);
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(f.name) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(f.kode_fasilitas) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    ...
    Logging(error, {
      file: "master/fasilitas/fasilitas_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
