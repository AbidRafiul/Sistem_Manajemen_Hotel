...
router.post("/", async (req, res) => {
  ...
  try {
    const baseQuery = DB("mst_amenity as a")
      .whereNull("a.deleted_at")
      .modify((qb) => {
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(a.name) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(a.kode_amenity) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    ...
    Logging(error, {
      file: "master/amenity/amenity_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
