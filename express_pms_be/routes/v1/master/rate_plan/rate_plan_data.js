...
router.post("/", async (req, res) => {
  ...
  try {
        ...
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(rp.nama_paket) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(rp.kode_paket_harga) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    const selectFields = [
      "rp.id",
      "rp.kode_paket_harga",
      "rp.nama_paket as name",
      "rp.tipe_paket",
      "rp.tipe_markup",
      "rp.nilai_markup",
      "rp.dapat_di_refund as bisa_refund",
      "rp.termasuk_sarapan",
      "rp.minimal_malam",
      "rp.maksimal_malam",
      "rp.is_active",
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery
        .clone()
        .select(selectFields)
        .orderBy(sortField === "name" ? "rp.nama_paket" : `rp.${sortField}`, sortOrder)
        .limit(perPage)
    ...
    Logging(error, {
      file: "master/rate_plan/rate_plan_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
