...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const cValidation = await validatePayload(
      { kode_cabang: Joi.string().required().label("Kode Cabang"),  name: Joi.string().required().label("Nama Paket"), tipe_paket: Joi.string().required().label("Tipe Paket"), bisa_refund: Joi.number().valid(0,1).optional().default(0).label("Bisa Refund"), termasuk_sarapan: Joi.number().valid(0,1).optional().default(0).label("Termasuk Sarapan"), minimal_malam: Joi.number().optional().default(1).label("Minimal Malam"), maksimal_malam: Joi.number().optional().allow(null).label("Maksimal Malam"), is_active: Joi.number().valid(0,1).optional().default(1).label("Status Aktif") },
    ...
    await DB.transaction(async (trx) => {
      ...
      const oData = { kode_cabang: oPayload.kode_cabang, kode_paket_harga: cUniqueCode, nama_paket: oPayload.name, tipe_paket: oPayload.tipe_paket, dapat_di_refund: oPayload.bisa_refund ?? 0, termasuk_sarapan: oPayload.termasuk_sarapan ?? 0, minimal_malam: oPayload.minimal_malam ?? 1, maksimal_malam: oPayload.maksimal_malam ?? null, is_active: oPayload.is_active !== undefined ? oPayload.is_active : 1, created_by: req.auth?.user_id || 1, created_at: formatDateSystem(), updated_at: formatDateSystem() };
