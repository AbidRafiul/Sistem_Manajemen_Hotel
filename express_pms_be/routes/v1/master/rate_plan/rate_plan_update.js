...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const oData = { kode_cabang: oPayload.kode_cabang, nama_paket: oPayload.name, tipe_paket: oPayload.tipe_paket, dapat_di_refund: oPayload.bisa_refund ?? existing.dapat_di_refund, termasuk_sarapan: oPayload.termasuk_sarapan ?? existing.termasuk_sarapan, minimal_malam: oPayload.minimal_malam ?? existing.minimal_malam, maksimal_malam: oPayload.maksimal_malam ?? existing.maksimal_malam, is_active: oPayload.is_active !== undefined ? oPayload.is_active : existing.is_active, updated_by: req.auth?.user_id || 1, updated_at: formatDateSystem() };
