  ...
  try {
    ...
    const baseQuery = DB("mst_kamar as r")
      ...
      .modify((qb) => {
        ...
        if (keyword)
          qb.where(function () {
            this.whereRaw("LOWER(r.nomor_kamar) LIKE ?", [`%${keyword.toLowerCase()}%`]).orWhereRaw(
              "LOWER(r.kode_kamar) LIKE ?",
              [`%${keyword.toLowerCase()}%`]
            );
          });
      });
    const selectFields = [
      ...
      "f.nama_lantai as floor_name",
      "rt.nama_tipe as room_type_name",
    ];
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery
        .clone()
        .select(selectFields)
        .orderByRaw(`${orderByCol} ${sortOrder}`)
        .limit(perPage)
    ...
    Logging(error, {
      file: "master/kamar/kamar_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
