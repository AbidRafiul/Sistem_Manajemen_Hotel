...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      {
        kode_tipe_ruang_event: Joi.string().required().label("Kode"),
    ...
    const existing = await DB("mst_tipe_ruang_event")
      .where("kode_tipe_ruang_event", oPayload.kode_tipe_ruang_event)
    ...
    await DB.transaction(async (trx) => {
      ...
      await ChangesLog(
        {
          description: "Update Master Tipe Ruang Event",
          tableName: "mst_tipe_ruang_event",
          referenceCode: oPayload.kode_tipe_ruang_event,
          action: "UPDATE",
          dataBefore: existing,
    ...
    Logging(error, {
      file: "master/tipe_ruang_event/tipe_ruang_event_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
