...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const cValidation = await validatePayload(
      { nama_tipe: Joi.string().required().label("Nama Tipe"), is_active: Joi.number().valid(0,1).optional().default(1).label("Status Aktif") },
    ...
    Logging(error, { file: "master/tipe_ruang_event/tipe_ruang_event_create.js", func: "create", request: oPayload, response: oResult, user: username });
