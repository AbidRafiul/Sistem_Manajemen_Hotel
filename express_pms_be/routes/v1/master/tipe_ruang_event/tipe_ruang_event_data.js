...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery.clone().select(selectFields).orderBy(`tre.${sortField}`, sortOrder).limit(perPage).offset((page - 1) * perPage);
    } else {
    ...
    Logging(error, { file: "master/tipe_ruang_event/tipe_ruang_event_data.js", func: "get", request: oPayload, response: oResult, user: username });
