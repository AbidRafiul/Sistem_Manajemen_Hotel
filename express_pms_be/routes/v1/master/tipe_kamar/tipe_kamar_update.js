...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const oData = { kode_cabang: oPayload.kode_cabang, nama_tipe: oPayload.name, kapasitas_dasar: oPayload.kapasitas_dasar ?? existing.kapasitas_dasar, kapasitas_maksimal: oPayload.kapasitas_maksimal ?? existing.kapasitas_maksimal, luas_sqm: oPayload.luas_m2 ?? existing.luas_sqm, deskripsi: oPayload.deskripsi ?? existing.deskripsi, is_active: oPayload.is_active !== undefined ? oPayload.is_active : existing.is_active, updated_by: req.auth?.user_id || 1, updated_at: formatDateSystem() };
