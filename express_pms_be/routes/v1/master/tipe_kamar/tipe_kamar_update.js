...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      {
        ...
        kode_tipe_kamar: Joi.string().required().label("Kode"),
    ...
    const existing = await DB("mst_tipe_kamar")
    ...
    const oData = {
      ...
      kode_bed_type:
        oPayload.kode_bed_type !== undefined ? oPayload.kode_bed_type : existing.kode_bed_type,
      kapasitas_dasar: oPayload.kapasitas_dasar ?? existing.kapasitas_dasar,
      kapasitas_maksimal: oPayload.kapasitas_maksimal ?? existing.kapasitas_maksimal,
      luas_sqm: oPayload.luas_m2 ?? existing.luas_sqm,
      harga_default: oPayload.harga_default ?? existing.harga_default,
      deskripsi: oPayload.deskripsi ?? existing.deskripsi,
    ...
    await DB.transaction(async (trx) => {
      ...
      await ChangesLog(
        {
          deskripsi: "Update Master Tipe Kamar",
          tableName: "mst_tipe_kamar",
          referenceCode: oPayload.kode_tipe_kamar,
          action: "UPDATE",
          dataBefore: existing,
    ...
    Logging(error, {
      file: "master/tipe_kamar/tipe_kamar_update.js",
      func: "update",
      request: oPayload,
      response: oResult,
      user: username,
