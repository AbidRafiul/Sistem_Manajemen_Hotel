...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      ...
      const existingData = await trx("mst_tipe_kamar").where("kode_tipe_kamar", cUniqueCode).first();
      if (existingData) {
        throw new Error("DUPLICATE_CODE");
      }
      
      await trx("mst_tipe_kamar").insert(oData);
      await ChangesLog({ deskripsi: "Tambah Master Tipe Kamar", tableName: "mst_tipe_kamar", referenceCode: cUniqueCode, action: "CREATE", dataBefore: null, dataAfter: oData, user: username, tz: oPayload.tz || "UTC" }, trx);
    });
    return res.status(200).json({ status: status.SUKSES, message: "Data Master Tipe Kamar berhasil dibuat", datetime: formatDateSystem(), data: { kode_tipe_kamar: cUniqueCode } });
  } catch (error) {
    ...
    Logging(error, { file: "master/tipe_kamar/tipe_kamar_create.js", func: "create", request: oPayload, response: oResult, user: username });
