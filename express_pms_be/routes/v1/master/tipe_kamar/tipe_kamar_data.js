...
router.post("/", async (req, res) => {
  ...
  const sortField = ["kode_tipe_kamar", "name", "is_active", "created_at", "updated_at"].includes(
    oPayload.sortField
  )
    ? oPayload.sortField
    : "updated_at";
  ...
  try {
    ...
    const selectFields = [
      ...
      "rt.nama_tipe as name",
      "rt.kode_bed_type",
      "bt.name as bed_type_name",
      "rt.kapasitas_dasar",
      "rt.kapasitas_maksimal",
      "rt.luas_sqm as luas_m2",
      "rt.harga_default",
      "rt.deskripsi",
      "rt.is_active",
      "rt.created_at",
    ...
    if (hasPagination) {
      ...
      vaData = await baseQuery
        .clone()
        .select(selectFields)
        .orderBy(sortField === "name" ? "rt.nama_tipe" : `rt.${sortField}`, sortOrder)
        .limit(perPage)
    ...
    Logging(error, {
      file: "master/tipe_kamar/tipe_kamar_data.js",
      func: "get",
      request: oPayload,
      response: oResult,
      user: username,
